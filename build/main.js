...
const isAllowUser_1 = require("./utils/isAllowUser");
(0, dotenv_1.config)();
...
async function start() {
    ...
    bot.use((ctx, next) => {
        ...
        if (userId && !(0, isAllowUser_1.isAllowUser)(userId)) {
            console.log(userId);
    ...
    bot.hears(constants_1.EXIT_BTN_TEXT, ctx => {
        ctx.replyWithHTML(`Натисни ще раз ${constants_1.EXIT_BTN_TEXT}`);
        return ctx.scene.enter(constants_1.REPORT_SCENE_ID);
    });
    bot.on("message", ctx => {
...
async function initSpreadSheet() {
    if (!process.env.SB_SPREADSHEET_ID)
        throw new Error("Spreadsheet ID not passed!");
    const doc = new google_spreadsheet_1.GoogleSpreadsheet(process.env.SB_SPREADSHEET_ID);
